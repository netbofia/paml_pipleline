...
parser.add_argument("-r",dest="run", help="none - paint | 1- Search for w values | 2- Tree search")
parser.add_argument("-s",dest="specie", help="Input species to search for")
###########################
...
#########TEST################
#infile="../Results/paml/Adam10Primate/wtree-Adam10.tree"
#run=None
#wdir=None
#############################
...
class walk(object):
    ...
    def comma(self, seq):
        ...
        for c in seq.split(","):
            ...
            if re.match(".*[a-zA-Z]+.*",c):
                ...
                self.outputTree+="\'"+leaf[0]
                self.species.append(leaf[0]+" "+"#"+leaf[1])
                self.tree.name=leaf[0]
    ...
    def color(self, seq):
        if float(seq)<=1:
            ...
            if self.tree.name is None:
                self.outputTree+="!hilight={"+str(self.branches(self.tree)+1)+",0.0,#-16763956}]:1.0"
            
        else:
            self.tree.color="blue"
            if self.tree.name is None:
                self.outputTree+="!hilight={"+str(self.branches(self.tree)+1)+",0.0,#-6750208}]:1.0"
        ...
        if score[0].name==name2:
            ...
            if score[0].name==name2:
                ...
                if score[0].prev is not None:
                    ...
                    result=self.t2([score[0].prev,0],name2)
                    if result[0].name==name2:
                        if self.debug:
    ...
    def up(self, score, name3):
        ...
        if score[0].name==name3:
            ...
            for t in score[0].node:
                result=self.up([t,0],name3)
                if result[0].name==name3:
                    score=result
            score[1]+=1
            return score
    def find(self, tree, name):
	if re.search(str(name), str(tree.name), re.IGNORECASE) is not None:
	    return tree
        else:
            for i in tree.node:
                result=self.find(i,name)
		if result is not None:
                    if re.search(str(name),str(result.name),re.IGNORECASE) is not None:
                        return result
      


#infile="../Results/paml/*Primate/*.tree"
...
if run=="1":
	...
	gene=i.rsplit("/",1)[1].replace("Ensembl.tree","").replace("wtree-","")
        print gene
        if wdir is None:
            tree=open(i,"r").read()
        else:
        ...
        parse=walk(tree)
        try:
	    tree=parse.find(parse.tree,specie)
            table.append([gene,tree.w,tree.prev.w])
	except:
	    table.append([gene,"-","-"])
    for line in table:
...
if run==2:
    ...
    for i in glob.glob(infile):
        if wdir is None:
            ...
            tree=open(wdir+i,"r").read()
        parse=walk(tree)
        first=parse.t1([parse.tree,0],name1)
        #print i.rsplit("/",2)[1].replace("Primate","")
        try:
            score1.append([i.rsplit("/",2)[1].replace("Primate",""),parse.t2([first[0],0],name2)[1],first[1]])
        except TypeError:
    ...
    for i in glob.glob(infile):
        if wdir is None:
            ...
            tree=open(wdir+i,"r").read()
        parse=walk(tree)
        first=parse.t1([parse.tree,0],name2)
        #print i.rsplit("/",2)[1].replace("Primate","")
        try:
            score2.append([i.rsplit("/",2)[1].replace("Primate",""),parse.t2([first[0],0],name1)[1],first[1]])
        except TypeError:
            error2.append(i.rsplit("/",2)[1].replace("Primate",""))
            print i.rsplit("/",2)[1].replace("Primate","") + " has an error *************"
